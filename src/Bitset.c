...
uint8_t bs_bitset_get_bit(uint8_t *a, uint64_t i)
{
    // return a[i];
    return a[i >> 3] & (1 << (i & 7));
}

uint8_t bs_bitset_get(uint8_t *a, uint64_t i)
{
    // return a[i];
    return (a[i >> 3] & (1 << (i & 7))) ? 1 : 0;
}

void bs_bitset_set(uint8_t *a, uint64_t i)
{
    // a[i] = 1;
    a[i >> 3] |= 1 << (i & 7);
}

void bs_bitset_unset(uint8_t *a, uint64_t i)
{
    // a[i] = 0;
    a[i >> 3] &= ~(1 << (i & 7));
}

void bs_bitset_maybe_set(uint8_t *a, uint64_t i, uint8_t bit)
{
    // a[i] = bit;
    if (bit)
        bs_bitset_set(a, i);
...
// void bs_bitset_for_each_bit(uint8_t *start, uint8_t *end, void *captures,
